...
def updateDatabaseRow(shot_database_row, hit_number):
    shot_database['hit_start_frame'][shot_database_row] = df['Frame'][prev_hit_row]
    shot_database['hit_end_frame'][shot_database_row] = df['Frame'][df.shape[0]-1]
    shot_database['hit_start_time'][shot_database_row] = df['Frame'][prev_hit_row]/30
    shot_database['hit_end_time'][shot_database_row] = df['Frame'][df.shape[0]-1]/30
    shot_database['mp4_filepath'][shot_database_row] = video_path
    shot_database['csv_filepath'][shot_database_row] = csv_path
    shot_database['set_number'][shot_database_row] = match_meta_data[1]

    shot_database['top_player'][shot_database_row] = match_meta_data[2]
    shot_database['top_player_score'][shot_database_row] = match_meta_data[3]
    shot_database['bottom_player'][shot_database_row] = match_meta_data[4]
    shot_database['bottom_player_score'][shot_database_row] = match_meta_data[5]
    ...
    if f.endswith("hits.csv"):
        ...
        for row in df.index:
            ...
            if df['Hit'][row] == 1:
                ...
                shot_database_row, hit_number = updateDatabaseRow(shot_database_row, hit_number)
